...
(function () {
  ...
  function resumenLocal(l) {
    ...
    var estadoVisual = l.estado === 'REFACCION' ? 'refaccion' : l.estado === 'LIBRE' ? 'libre' : (vencidas.length ? 'deuda' : 'aldia');
    ...
    return { contrato: vig, deuda: deuda, totalDeuda: totalDeuda, vencidas: vencidas, mantPend: mantPend, estadoVisual: estadoVisual,
      diasVenc: vig ? diasHasta(vig.fecha_fin) : null, diasAjuste: vig && vig.proxima_fecha_ajuste ? diasHasta(vig.proxima_fecha_ajuste) : null };
  }
  var NOMBRE_ESTADO = { libre: 'Libre', aldia: 'Alquilado · al día', deuda: 'Alquilado · con deuda', refaccion: 'En refacción' };

  function localesFiltrados() {
    var t = textoBusqueda.toLowerCase();
    return D.locales.filter(function (l) { return l.activo !== false; }).map(function (l) { return { l: l, r: resumenLocal(l) }; }).filter(function (x) {
      if (filtroEstado === 'mant' && !x.r.mantPend.length) return false;
      if (['libre', 'aldia', 'deuda', 'refaccion'].indexOf(filtroEstado) >= 0 && x.r.estadoVisual !== filtroEstado) return false;
      if (t) {
  ...
  function renderKpis() {
    ...
    $('kpis').innerHTML =
      ...
      kpi('m² alquilados', m2Alq.toLocaleString('es-AR'), 'de ' + m2Tot.toLocaleString('es-AR') + ' m² · libres: ' + (m2Tot - m2Alq).toLocaleString('es-AR'), '') +
  ...
  function renderFiltros() {
    var cnt = { todos: 0, libre: 0, aldia: 0, deuda: 0, refaccion: 0, mant: 0 };
    D.locales.forEach(function (l) { var r = resumenLocal(l); cnt.todos++; cnt[r.estadoVisual]++; if (r.mantPend.length) cnt.mant++; });
  ...
  function renderLista() {
    ...
    $('lista-locales').innerHTML = items.map(function (x) {
      var det = x.r.contrato ? x.r.contrato.inquilino : (x.l.estado === 'REFACCION' ? 'En refacción' : 'Libre · ' + x.l.m2 + ' m²');
      var tag = x.r.totalDeuda > 0 ? fmtMonto(x.r.totalDeuda) : (x.r.mantPend.length ? '🔧 ' + x.r.mantPend.length : '');
      return '<li data-id="' + x.l.id_local + '" class="' + (localSel === x.l.id_local ? 'sel' : '') + '">' +
        '<span class="punto sw-' + x.r.estadoVisual + '"></span>' +
        '<div><div class="nom">' + esc(x.l.nombre) + ' <span style="color:var(--texto-3);font-weight:400">· ' + nombrePlanta(x.l.planta) + '</span></div><div class="det">' + esc(det) + '</div></div>' +
        '<div class="tag">' + tag + (x.r.mantPend.length && x.r.totalDeuda > 0 ? '<br>🔧 ' + x.r.mantPend.length : '') + '</div></li>';
    }).join('');
    $('lista-locales').querySelectorAll('li').forEach(function (li) { li.onclick = function () { abrirFicha(li.dataset.id); }; });
  }
  ...
  async function cargarPlano() {
    ...
    zonas.forEach(function (z) {
      ...
      z.onclick = function () { if (l) abrirFicha(l.id_local); };
      var bb = z.getBBox();
      var g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('class', 'etiqueta'); g.setAttribute('data-for', z.id);
      var fs = Math.max(11, Math.min(18, bb.width / 8));
      g.innerHTML = '<text x="' + (bb.x + bb.width / 2) + '" y="' + (bb.y + bb.height / 2 - 2) + '" text-anchor="middle" font-size="' + fs + '">' + esc(l ? l.nombre : z.id) + '</text>' +
        '<text class="sub" x="' + (bb.x + bb.width / 2) + '" y="' + (bb.y + bb.height / 2 + fs) + '" text-anchor="middle" font-size="' + (fs * .72) + '" data-sub="' + z.id + '"></text>';
      ...
      badge.innerHTML = '<circle cx="' + (bb.x + bb.width - 14) + '" cy="' + (bb.y + 14) + '" r="12"/><text x="' + (bb.x + bb.width - 14) + '" y="' + (bb.y + 19) + '" text-anchor="middle" font-size="13">🔧</text>';
  ...
  function pintarMapa() {
    ...
    svg.querySelectorAll('.zona').forEach(function (z) {
      ...
      if (sub) sub.textContent = r.contrato ? r.contrato.inquilino : (l.estado === 'REFACCION' ? 'refacción' : 'libre · ' + l.m2 + ' m²');
      var t = z.querySelector('title'); if (t) t.textContent = l.nombre + ' · ' + NOMBRE_ESTADO[r.estadoVisual] + (r.totalDeuda ? ' · debe ' + fmtMonto(r.totalDeuda) : '') + (r.mantPend.length ? ' · ' + r.mantPend.length + ' trabajo(s) pendiente(s)' : '');
    });
  }
  ...
  function abrirFicha(id, tab) {
    ...
    if (l && l.planta !== plantaActual) { cambiarPlanta(l.planta); }
  ...
  function renderFicha() {
    ...
    $('ficha-sub').textContent = l.m2 + ' m²' + (l.rubro ? ' · ' + l.rubro : '');
